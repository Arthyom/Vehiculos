      ...
      <tr v-for="(provider,i) in allProviders">
        ...
        <td>
          <div class=" items-center gap-3">
            <div class=" avatar">
              <div class="mask mask-squircle h-25 w-25 bg-gray-200">
                <template v-if="!!provider">
                    <img
                      :src="provider.imagenes || providerNoImage"
                      alt="Avatar Tailwind CSS Component" />
                </template>
              </div>
            </div>
            <div>
              <div class="font-bold">{{ provider.Alias }} </div>
              <div class="text-sm opacity-50">{{ provider.Nombre }}</div>
            </div>
          </div>
        </td>
        <td>
            <span class="badge badge-ghost badge-md">{{ provider.Descripcion }}</span>
        </td>

        <td>
            <span class="badge badge-ghost badge-md">{{ provider.Direccion }} km</span>
        </td>

        <th>
         <LinksTable to="proveedores" :item="provider"></LinksTable>
        </th>
      </tr>
